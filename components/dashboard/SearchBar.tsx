            ...
            <CommandGroup heading='Users'>
              {queryResults?.map((user) => (
                <CommandItem
                  ...
                  key={user.id}
                  value={user.email as string || user.name as string || user.bio as string || user.profession as string}>
                  ...
                  <a href={`/user-profile/${user.id}`} className=""><span className="text-slate-500">{user.name} {user.profession}</span> ... <span className="text-[#faa635]">{user.role}</span></a>
                </CommandItem>
              ))}
            </CommandGroup>
          ) : null}
